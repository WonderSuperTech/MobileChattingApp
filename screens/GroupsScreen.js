...
import { FAB } from 'react-native-paper';  // Assuming you're using react-native-paper for FAB
...
const GroupsScreen = () => {
  ...
  useEffect(() => {
    ...
    const q = query(collectionRef,
      where('users', "array-contains", { email: auth?.currentUser?.email, name: auth?.currentUser?.displayName, profileImage: auth?.currentUser?.photoURL, deletedFromChat: false, isAdmin: true || false }),
  ...
  const handleGroupImage = (chat) => {
    if (chat.data().groupImage) {
      return chat.data().groupImage;
    }
    return "";
  };
  ...
  const handleOnPress = (chat) => {
    ...
    navigation.navigate('GroupChat', { id: chat.id, chatName: handleChatName(chat), chatImage: handleGroupImage(chat) });
  };
    ...
    <Pressable style={styles.container} onPress={deSelectItems}>
      {loading ? (
        ...
        <View style={styles.blankContainer}>
          <Text style={styles.textContainer}>No Groups yet</Text>
        </View>
      ) : (
        <ScrollView>
          {chats.map(chat => (
      ...
      <FAB
        style={styles.fab}
        small
        icon={disableChatFab ? "delete" : "account-multiple-plus"}
        color="#fff"
